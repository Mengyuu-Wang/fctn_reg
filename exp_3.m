...
for s = 1:2  
    ...
    for R_ind = 1  % rank 组
        ...
        for n_ind = 1:length(ns)
            ...
            rmse_list_fctn_sample = zeros(reps, 1);
            runtime_list_fctn_sample  = zeros(reps, 1);
            ...
            for i = 1:reps
                ...
                %% run fctn_sampled
                J = 2500;
                [model, runtime] =  fctn_reg_rand(para, rk_fctn, J, trainx, trainy, valx, valy);
                est_testy_fctn = contract(testx, model, para.L);
                Ypred = zscore(est_testy_fctn(:));

                rmse_list_fctn_sample(i) = sqrt(mean((Ypred - Ytrue).^2));
                runtime_list_fctn_sample(i) = runtime;
            end
            ...
            RMSEmat_fctn_sample(group_id, n_ind)  = mean(rmse_list_fctn_samle);
            TIMEmat_fctn_sample(group_id, n_ind)  = mean(runtime_list_fctn_samle);
