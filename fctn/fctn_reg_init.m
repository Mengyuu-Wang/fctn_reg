function [best_model,best_runtime,trainerror ] = fctn_reg_init( para, rank, X,Y,Xv,Yv, init,Replicates)
%% set parameters
...
elseif init == 0
    %initialization with fctn-svd
    Y_mat = reshape(Y,NN,numel(Y)/NN);
    X_mat = reshape(X,NN,numel(X)/NN);
    model_mat = Y_mat \ X_mat;
    sz_model = horzcat(para.P,para.Q);
    tmp_model = reshape(model_mat.',sz_model);
    [~,cores] = FCTN_svd_setrank(tmp_model,rank);
    tmp_cores = cores.';
end
re_cores = cell(N,1);
pp = cell(N,1);
